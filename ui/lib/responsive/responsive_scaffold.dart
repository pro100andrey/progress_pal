...
// The maximum media size for phone mode.
// Below or equal to this breakpoint the menu toggles between
// hidden in the Drawer and rail, also on phones. This is just the default
// value for the constructor and it can be set differently in the
// ResponsiveScaffold constructor.
const double _kPhoneWidthBreakpoint = 600;
...
enum MenuState { none, drawer, rail, menu }
...
typedef SectionBuilder =
    Widget? Function(BuildContext context, MenuState menuState);
...
class ResponsiveScaffold extends StatefulWidget {
  const ResponsiveScaffold({
    ...
    this.desktopWidthBreakpoint = _kDesktopWidthBreakpoint,
    this.phoneWidthBreakpoint = _kPhoneWidthBreakpoint,
  ...
  /// Optional header widget displayed above the menu items.
  final SectionBuilder? header;

  /// Optional footer widget displayed below the menu items.
  final SectionBuilder? footer;
  ...
  ///
  /// Defaults to [_kDesktopWidthBreakpoint] 900.
  final double desktopWidthBreakpoint;

  /// Breakpoint when we want to show the phone sized menu.
  ///
  /// Below this breakpoint, we can toggle between rail and drawer view, over
  /// and equal to this breakpoint, we can toggle between full sized menu
  /// or rail, we never see the drawer over this breakpoint.
  final double phoneWidthBreakpoint;
  ...
  ///
  /// These are typically [IconButton] widgets.
...
class _ResponsiveScaffoldState extends State<ResponsiveScaffold> {
  ...
  void didUpdateWidget(covariant ResponsiveScaffold oldWidget) {
    ...
    if (mediaSize != previousMediaSize) {
      ...
      final isPhone = mediaSize.width < widget.phoneWidthBreakpoint;
  ...
  void initState() {
    ...
    menuItemsEnabled = List<bool>.generate(
    ...
    menuItemsIconState = List<ResponsiveMenuItemIconState>.generate(
      widget.menuItems.length,
      (i) => ResponsiveMenuItemIconState.primary,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final isDesktop =
        MediaQuery.sizeOf(context).width >= widget.desktopWidthBreakpoint;

    activeMenuWidth = isDesktop
        ? (isMenuExpanded ? widget.menuWidth : widget.railWidth)
        : (isMenuClosed ? 0 : widget.railWidth);

    MenuState menuState() {
      if (_scaffoldKey.currentState?.isDrawerOpen ?? false) {
        return MenuState.drawer;
      } else if (activeMenuWidth == widget.menuWidth) {
        return MenuState.menu;
      } else if (activeMenuWidth == widget.railWidth) {
        return MenuState.rail;
      } else {
        ...
        RepaintBoundary(
          child: FocusTraversalGroup(
            child: ConstrainedBox(
              ...
              // Material is used for proper surface, and it gives us the
              // "right" theme behaving container background.
              child: Material(
                // This simple implicit animation AnimatedContainer is what
                // animates the entire side menu from 0, to rail to menu width.
                // The default type, canvas, makes Material use
                // theme.CanvasColor, which in FlexThemeData and in
                // ThemeData.from is set to theme.colorScheme.background.
                // So our menu will be background colored by default,
                // including any color branding our theme in FlexColorScheme
                // has applied to it, just like a Drawer.
                child: AnimatedContainer(
                  duration: _kMenuAnimationDuration,
                  onEnd: () {
                    setState(() {
                      // This state was added to not get the automatically
                      // implied leading widget to show up before the menu rail
                      // completed sliding out of visibility, looked better that
                      // way. Otherwise it jumps into visibility when the menu
                      // rail starts its closing animation, and heading jumps to
                      // right.
                      if (isMenuClosed) {
                  ...
                  child: _AppMenu(
                    header: (c) => widget.header?.call(c, menuState()),
                    footer: (c) => widget.footer?.call(c, menuState()),
                    ...
                    // User pushed the menu button, change menu state, on
                    // desktop we toggle the the menuExpanded and not
                    // menuExpanded state. On not desktop size (phone or tablet)
                    // we toggle the state isMenuClosed or not.
                    onOperate: () {
                      setState(() {
                        // Desktop case, we can only expand or collapse the
                        // menu.
                        if (isDesktop) {
                          // So we toggle its state.
                          isMenuExpanded = !isMenuExpanded;
                        } else {
                          // Tablet or phone case, we can only close the menu,
                          // it will then be in the Drawer, from where it can be
...
class _AppMenuState extends State<_AppMenu> {
  ...
  void initState() {
    ...
    selectedItem = widget.selectedIndex ?? 0;
  }
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, size) {
      ...
      final menuBackground = theme.canvasColor;
      final scaffoldBackground = theme.scaffoldBackgroundColor;
      final closeColors = colorsAreClose(
        menuBackground,
        scaffoldBackground,
        isLight: isLight,
      );
                ...
                child: ClipRect(
                  child: OverflowBox(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, i) => _MenuItem(
                        selectionEnabled: widget.selectedIndex != null,
                        width: size.maxWidth,
...
class _MenuItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final iconColor = _getColor(theme, isLight, enabled);
    final textColor = _getTextColor(theme, enabled);
            ...
            child: Stack(
              children: [
                Material(
                  ...
                  // However, you can do this on a menu that selects an item
                  // that should remain highlighted and show last selected item:
                  // color: selected ? theme.focusColor : Colors.transparent,
                  color: selectionEnabled && selected
                      ? theme.focusColor
                      : Colors.transparent,
  ...
  Color _getColor(ThemeData theme, bool isLight, bool enabled) {
    if (!enabled) {
      return theme.colorScheme.onSurface.withAlpha(0x55);
    }
  ...
  Color _getTextColor(ThemeData theme, bool enabled) => enabled
